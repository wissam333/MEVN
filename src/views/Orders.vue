  ...
  <div class="contain" :class="sidebar ? 'shrink' : ''">
    <div class="title-container">
      <div class="head-title">
        <h4 class="header4">Orders List</h4>
      </div>
      <div class="options">
        <div class="reload btn btn-success" @click="getOrdersTrigger()">
    ...
    <div class="table-responsive">
      ...
      <div v-else class="User-tabel">
        <table>
          ...
          <tbody>
            <tr class="rows" v-for="order in orders" :key="order._id">
              <td>
                <p class="">
                  {{ order.name }}
                </p>
              ...
              <td>
                <span class="delete" @click="confirmDelete(order._id)">
...
// store
import { golbalVar, Orders } from "../stores/counter";
import { storeToRefs } from "pinia";
const getGolbalVar = golbalVar();
const OrdersData = Orders();
const { sidebar } = storeToRefs(getGolbalVar);
const { getOrdersLoading } = storeToRefs(OrdersData);

let orders = ref([]);
// get Orders
const getOrdersTrigger = async () => {
  orders.value = await OrdersData.getOrders();
};
onBeforeMount(() => {
  getOrdersTrigger();
});
...
let confirmDelete = (UserId) => {
  deleteUserId.value = UserId;
  deleteModal.value = true;
};
let DeleteUserTrigger = async () => {
  deleteModal.value = false;
  const res = await OrdersData.deleteOrder(deleteUserId.value);
  if (res.status === 200) {
...
let DeleteUserTemp = () => {
  orders.value = orders.value.filter((item) => item._id !== deleteUserId.value);
};
